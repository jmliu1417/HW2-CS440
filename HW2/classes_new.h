...
public:
    ...
    { // Write a Record into your page (main memory)

        if (cur_size + r.get_size() >= 4096)
        { // Checking if the current Record can be entered or not
    ...
    void write_into_data_file(ostream &out) const
    {
        ...
        { // TO_DO: Write the slot-directory information into page_data. You'll use slot-directory to retrieve record(s).
            ...
            else
            {
                ...
                memcpy(page_data + offset + sizeof(slots.first), &slots.second, sizeof(slots.second));
        ...
        { // Read each line from the CSV file, parse it, and create Employee objects
            ...
            { // inserting that record object to the current page
                ...
                { // Checking if page limit has been reached.
                    ...
                    page_number = 0; // Reset page_number to 0 after writing all pages
                }
                for (page &p : buffer)
                {
                    // Checking for remaining records
                    if (p.cur_size > 0)
                    {
                        p.write_into_data_file(data_file);
